#ifndef FEATURES_H
#define FEATURES_H

// QUẢN LÝ TỆP VÀ THƯ MỤC (9)
#include "directory.h" // 4 lệnh
#include "file.h"      // 5 lệnh

// QUẢN LÝ TIẾN TRÌNH (11)
#include "process.h"        // 11 lệnh
#include "manage_threads.h" // 1 lệnh

// HỆ THỐNG VÀ TIỆN ÍCH (17)
#include "run_script.h"   // 1 lệnh
#include "system_utils.h" // 7 lệnh
#include "scheduler.h"    // 1 lệnh
#include "alias.h"        // 3 lệnh
#include "bookmark.h"     // 4 lệnh
#include "set_color.h"    // 1 lệnh

// TÍNH TOÁN NÂNG CAO (18)
#include "variables.h"       // 0 lệnh
#include "dancing.h"         // 0 lệnh
#include "ronaldo.h"
#include "random_fact.h"
#include "function.h"        // 2 lệnh
#include "net_speed.h"
#include "solve_quadratic.h"
#include "conditional.h"     // 1 lệnh
#include "environment.h"     // 9 lệnh
#include "loop.h"            // 1 lệnh
#include "base_conversion.h" // 1 lệnh

// ĐIỀU HƯỚNG VÀ GIAO DIỆN (10)
#include "navigation.h"   // 5 lệnh
#include "exit_shell.h"   // 1 lệnh
#include "help.h"         // 1 lệnh
#include "history.h"      // 2 lệnh
#include "clear_screen.h" // 1 lệnh

#endif // FEATURES_H
