...
public:
    ...
    void startProcessForeground(const std::vector<std::string> &args)
    {
        if (args.size() < 1)
        {
            std::cerr << "Usage: run_fg <command>" << std::endl;
            return;
        }
    ...
    void startProcessBackground(const std::vector<std::string> &args)
    {
        if (args.size() < 1)
        {
            std::cout << "Usage: run_bg <executable_path> [arguments...]" << std::endl;
            return;
        }
    ...
    void startRonaldo()
    {
        std::vector<std::string> args = {"Process/ronaldo.exe"};
        startProcessForeground(args);
    }

    void startProducerConsumerWinApi(const std::vector<std::string>& params_from_shell) {
        std::vector<std::string> exec_args;
        exec_args.push_back("Process/producer_consumer.exe"); // Tên chương trình

        // Thêm các tham số từ shell vào
        for (const std::string& param : params_from_shell) {
            exec_args.push_back(param);
        }
        // Gọi startProcessForeground với các đối số đã xây dựng
        startProcessForeground(exec_args);
    }
